	...
	attach: function(attachTo, toggle){
		...
		var getOpenEvent = function(element){
			return function(event){
				// event.stop(); // event delegation fix - AJN
				var tag = event.target.get('tag');
				if (event.type === 'focus') {
					return;
				}

				if (self.opened && self.input == element) {
					event.stop();
					self.close();
					return;
				}
...
MD.DatePicker = new Class({
	Extends: Picker.Date
	,Implements: [Options,Events]
	,options: {
		// Picker
		///////////////////////////////
		pickerClass: 'MDDatePicker'
		,inject: null
		,animationDuration: 400
		,useFadeInOut: true
		,positionOffset: {x: 0, y: 0}
		...
		,updateAll : false //whether or not to update all inputs when selecting a date
		,weeknumbers: false
		,convertInputValueToDate: null // function(input)
	}

	,initialize: function(element, options){
		this.setOptions(options);

		if ( this.options.pickerClass != 'MDDatePicker') {
			this.options.pickerClass = 'MDDatePicker '+this.options.pickerClass;
		}
	...
	,getInputDate: function(input){
		...
		if (!input) return;
		var date;
		if (this.options.convertInputValueToDate) {
			date = this.options.convertInputValueToDate.call(this, input);
		} else {
	...
	,position: function(x, y){
		...
		if (typeOf(x) == 'element'){
			...
			var elementCoords = element.getCoordinates((this.options.inject ? this.options.inject : null));
	...
	,open: function(noFx){
		if (this.opened == true) return this;
		// this.position(); Caused error in ie8, don't think it's necessary - AJN
		this.opened = true;
		var picker = this.picker.setStyle('display', 'block').set('aria-hidden', 'false')
		if (this.shim) this.shim.show();
		this.fireEvent('open');
